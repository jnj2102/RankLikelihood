...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Simulation combination: n100, p50, sparsity = fivepercent
...
%save('RankLikelihood_Sigma_true_p50_n100_fivepercent.mat','sigma_true');

%save the omega matrix for R
%save('RankLikelihood_Omega_true_p50_n100_fivepercent.mat','omega_true');

%set the number of replications for the simulation
reps = 100;

    y_true_n100_p50 = cell([reps,1]);

    optimalJ_n100_p50_fivepercent = cell([reps,1]);
    minK_n100_p50_fivepercent = cell([reps,1]);
    Final_AIC_n100_p50_fivepercent = cell([reps,1]);
    

 x_matrix_n100_p50 =  cell([reps,1]);
...
MLE_ald_n100_p50= cell([reps,1]);
    MLE_ev_n100_p50 = cell([reps,1]);
    MLE_stable_n100_p50 = cell([reps,1]);
    MLE_log_n100_p50 = cell([reps,1]);
...
for iters = 1:reps
    ...
    % generate the true y using the fivepercent model
    y_true = mvnrnd(mean_true,sigma_true,n); 
    
    
    %Save the true y
    y_true_n100_p50{iters} = y_true;
     
   
    [x_matrix, MLE_ald,MLE_ev, MLE_log, MLE_stable] = xfunctions_ranklikelihood(p, y_true);
    
    x_matrix_n100_p50{iters} = x_matrix;
    MLE_ev_n100_p50{iters} = MLE_ev;
    MLE_stable_n100_p50{iters} = MLE_stable;
    MLE_ald_n100_p50{iters} = MLE_ald;
    MLE_log_n100_p50{iters} =  MLE_log;


    
    %save the x matrix as a mat file for R
    
   % save(sprintf('RankLikelihood_Iter_%d_X_matrix_p50_n100_fivepercent.mat', iters), 'x_matrix');
...
N_points);
    ...
    optimalJ_n100_p50_fivepercent{iters} = optimalJ;
    minK_n100_p50_fivepercent{iters} = minK;
    Final_AIC_n100_p50_fivepercent{iters} = Final_AIC;
...
save('RankLikelihood_p50_n100_fivepercent_prior.mat');
